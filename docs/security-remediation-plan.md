...
| **G2** | #4 처방자 위조 차단 | ✅ **머지 완료** | PR #70 ← `fix/#65` | 계획에 없던 `doctor-patients` 건도 함께 처리 — §4 이슈 #4 하단 참조 |
| **G2** | #5 타임아웃 + 트랜잭션 | ✅ **머지 완료** | PR #70 ← `fix/#65` | 외부 호출을 트랜잭션 밖으로 분리. 무응답 시 503으로 끊김 — §4 이슈 #5 하단 참조 |
| **G2** | #6 역할 기반 인가 | ✅ **머지 완료** | PR #70 ← `fix/#65` | 전 컨트롤러 `@PreAuthorize`. 인가 거부가 500으로 새던 것도 403으로 정정 — §4 이슈 #6 하단 참조 |
| **G2** | FastAPI 공유 시크릿 헤더 | ✅ **구현 완료 · PR 대기** | `fix/#66` | `X-Internal-Secret` + `hmac.compare_digest`. 미설정 시 FastAPI 기동 거부 — §3 G2 하단 참조 |
| **G2** | LIKE 와일드카드 이스케이프 | ✅ **구현 완료 · PR 대기** | `fix/#66` | 계획에는 환자 검색만 있었으나 질병·약품 검색에도 같은 결함 — 4곳 전부 처리 |
| **배포** | 운영 설정 분리 | ✅ **구현 완료 · PR 대기** | `fix/#66` | `docker-compose.prod.yml`, `DB_PASSWORD` 필수화, `.env.production.example`, EC2 배포 가이드 |
| **G2** | 미인증 응답 401 정정 | ✅ **구현 완료 · PR 대기** | `fix/#66` | 빈 본문 403 → JSON 401. **프론트가 세션 만료와 권한 부족을 구분할 수 있게 되는 전제** — §2-3 하단 "401 전역 처리" 정정 참조 |
| **프론트** | 오류 처리·세션 만료 대응 | ✅ **구현 완료 · PR 대기** | `fix/#66` | 백엔드 #1~#6 변경에 맞춘 프론트 정비 — §3 하단 "프론트엔드 정비" 참조 |
| **G3/G4** | — | ⬜ 착수 전 | — | HTTPS는 배포 가이드 8단계에 절차만 마련 |

> #1~#6은 `main`에 머지 완료. 아래 §2의 지적 중 ✅ 표시가 없는 항목은 현재 `main`에서 여전히 유효하다.
> `fix/#66`의 5건은 아직 PR 전이다.
...
#### ⚠️ (D) 공통 `AccessGuard` AOP 권고 — 비용 과다 · ✅ **수정 완료(이슈 #6)**
...
> **구현 시 계획과 달라진 점 — ArchUnit을 쓰지 않았다.**
> 같은 보장을 얻으면서 Gradle 의존성을 늘리지 않는 방법이 있어 그쪽을 택했다.
> `EndpointAuthorizationTest`가 스프링이 **실제로 등록한** `RequestMappingHandlerMapping` 핸들러 목록을 읽어,
> 우리 패키지의 모든 핸들러가 인가 애노테이션을 갖고 있는지 확인한다.
> ArchUnit은 바이트코드를 훑어 "컨트롤러처럼 생긴 클래스"를 추정하지만, 이 방식은 등록된 엔드포인트 그 자체를 본다 —
> 등록 안 된 코드는 애초에 대상이 아니고, 등록됐는데 빠진 것은 반드시 걸린다.
...
- **`KioskService.java:82-86` 대상 선정** — ✅ **해결 (이슈 #5 브랜치에 포함)**
  루프에 `break`가 없어 마지막 후보를 잡는데, 리포지토리 메서드가
  `findByStatusOrderByVisitDateAsc`(**오래된 순**)이라 마지막 원소는 **가장 최근 접수 환자**다.
  Javadoc은 *"가장 최근"*이라 적혀 있어, 정렬 방향과 코드가 서로 다른 이야기를 하고 있었다.

  **어느 쪽이 맞는지 프론트로 확정했다 → "가장 최근"이 맞다.**
  대기실 태블릿은 이 API를 **3초마다 폴링**하다가 대상이 생기면 즉시 이동한다
  (`KioskWaiting.tsx:83-94`). 즉 태블릿 앞에 서 있는 사람은 방금 접수한 환자다.
  오래된 순으로 바꾸면, **접수만 하고 태블릿을 쓰지 않은 묵은 접수가 하나라도 남는 순간
  태블릿이 그 접수를 영원히 반복해 잡아 새 환자가 아무도 진입하지 못한다.**
  `break` 한 줄로 오래된 순을 택하는 것은 고치는 게 아니라 기능을 망가뜨리는 변경이었다.

  → **동작은 그대로 두고 의도를 코드에 드러냈다.** `VisitRepository.findLatestWithoutPreliminaryAnalysis`
  (JPQL `NOT EXISTS` + `ORDER BY visitDate DESC` + `Limit.of(1)`)로 교체해
  **감사 문서의 N+1 지적도 같은 수정에서 함께 해소**했다(행마다 `existsByVisitId` → 조회 1번).
  회귀 테스트: `DiagnosisApplicationTests.kioskAutoEntryPicksLatestVisitWithoutPreliminaryAnalysis`
  — 묵은 접수가 있어도 최신 접수를 잡고, 이미 예비분석을 마친 건은 건너뛰는지 고정한다.
...
| 이름 검색 `%` 미이스케이프 | `PatientService.java:61` | ✅ 검색어 `%` 입력 시 전체 명단 덤프. **범위 정정**: 질병·약품 검색에도 동일 결함 (§3 G2 하단) |
...
| N+1 (대기 환자마다 exists) | `KioskService.java:82-86` | ✅ → **수정됨 `fix/#65`** (조회 1번짜리 JPQL로 교체 — K항) |
...
| 401 전역 처리 없음 | `client.ts` | ✅ → **`fix/#66`에서 처리** (아래 참조) |
| PrivateRoute가 role 미확인 | `PrivateRoute.tsx:6` 로그인 여부만 확인 | ✅ → **이슈 #6에서 처리** (`RoleRoute`) |
| 토큰 갱신 없음 | `jwt.expiration-ms` 24시간 고정 | ✅ (만료 시 자동 로그아웃까지는 `fix/#66`에서 처리) |
| `Clinic.tsx` 단일 컴포넌트 | 1,101줄 | ✅ |
| 요청 타임아웃·재시도·취소 없음 | `client.ts:27` | ✅ |
| ErrorBoundary 없음 | 전체 검색 0건 | ✅ |
| `alert()` 2곳 | `Login.tsx:30,46` | ✅ → **`fix/#66`에서 인라인 표시로 교체** |
| ~~백엔드 테스트 2개~~ / 프론트 0개 | `DiagnosisApplicationTests.java` | 🔧 **정정: 백엔드도 0개** — `compileTestJava` 자체가 실패한다 (J항) |

</details>

#### 🔧 정정 — "401 전역 처리 없음"의 진짜 원인은 프론트가 아니었다

감사 문서는 이 항목을 **프론트 과제**로 분류했다. 실제로 고치려고 들어가 보니
**프론트만으로는 고칠 수 없는 항목**이었다.

측정 결과, 로그인하지 않은 요청에 서버가 돌려주던 응답은 이랬다:
...
`401`이 아니라 **본문이 비어 있는 `403`** 이다. `SecurityConfig`에 `exceptionHandling`이 없어서
Spring Security의 기본 진입점(`Http403ForbiddenEntryPoint`)이 그대로 쓰이고 있었다.
여기서 두 가지가 동시에 망가진다:

| 증상 | 왜 |
|---|---|
| 세션 만료를 감지할 수 없다 | 권한 부족(`@DoctorAccess` 거부)도 `403`이라 **두 상황이 같은 코드로 온다.** 만료면 재로그인시켜야 하고 권한 부족이면 안내만 해야 하는데, 구분할 근거가 응답에 없다 |
| 오류 메시지가 화면에 안 뜬다 | 본문이 비어 `apiRequest`가 `message: ""`인 `ApiError`를 만든다. 사용자는 버튼을 눌렀는데 **아무 반응이 없는 것처럼** 본다 |

그래서 프론트 작업 전에 백엔드에 `SecurityErrorResponder`를 먼저 넣었다
(미인증 → `401` JSON / 필터 단계 권한 거부 → `403` JSON, 본문 형태는 `GlobalExceptionHandler`와 동일).

> 📌 **이 계획서를 쓰는 사람에게.** "프론트 항목"으로 적힌 것이 실제로는 백엔드 계약 문제인 경우가 있다.
> 프론트가 할 수 있는 일은 **서버가 구분해서 준 정보를 쓰는 것**뿐이고, 서버가 구분해 주지 않으면
> 프론트에서 아무리 정교하게 처리해도 추측밖에 못 한다. 감사 문서의 계층 분류를 그대로 믿지 말 것.
...
| 감사 4번 인가 미강제 | **(D)항 채택**: `@PreAuthorize` 전면 적용 + 애노테이션 누락 시 빌드 실패시키는 테스트 1개. **소유권 검사는 넣지 않는다 — (C)항 참조** | ✅ 이슈 #6 |
| 감사 8번 FastAPI 노출 | compose에서 `ports` 제거(내부 통신만) + 백엔드↔FastAPI 공유 시크릿 헤더 | ✅ 포트 제거(이슈 #3) + 시크릿 헤더(`fix/#66`) |
...
| `PatientService.java:61` | `%`·`_` 이스케이프 처리 | ✅ `fix/#66` — **환자·질병·약품 4곳 전부** |
...
#### G2 잔여 2건 실제 구현 (`fix/#66`)

**① FastAPI 공유 시크릿 헤더**

| 파일 | 변경 |
|---|---|
| `fastapi/main.py` | 기동 시 `INTERNAL_API_SECRET` 미설정이면 `RuntimeError`로 **기동 거부**. `verify_internal_secret` 의존성을 `/predict`·`/predict-base64`에 부착 |
| `AnalysisService` / `KioskService` | `@Value("${fastapi.internal-secret}")` → 요청에 `X-Internal-Secret` 헤더 첨부 |
| `application.properties` | `fastapi.internal-secret=${INTERNAL_API_SECRET}` — **기본값 없음** |
| `docker-compose.yml` | fastapi에 `${INTERNAL_API_SECRET:?...}`, backend에 같은 값 주입 |

> 📌 **`==` 대신 `hmac.compare_digest`를 쓴 이유**: 문자열 비교는 앞에서부터 맞춰보다 틀리는 순간 멈춘다.
> 그래서 첫 글자만 맞아도 응답이 **아주 조금** 늦어진다. 이 차이를 수천 번 측정하면 시크릿을 한 글자씩
> 복원할 수 있다(타이밍 공격). `compare_digest`는 항상 같은 시간이 걸린다.
> 로컬 네트워크 한정이라 실효 위험은 낮지만, 대안의 비용이 import 한 줄이라 안 쓸 이유가 없다.

> 📌 **`/health`는 열어 뒀다.** 헬스체크는 시크릿 없이 도달할 수 있어야 도커·로드밸런서가 쓸 수 있고,
> 노출되는 정보는 "떠 있다"뿐이다. 코드에 그 판단을 주석으로 남겼다.

**② LIKE 와일드카드 이스케이프 — 계획서 범위를 넘겨 처리했다**

계획서에는 `PatientService.java:61` 한 곳만 적혀 있었으나, 실제로 확인해 보니
**질병 검색(`KcdDiseaseController`)과 약품 검색(`DrugMasterController`)에 같은 결함**이 있었다.
공용 유틸을 어차피 만드는 김에 4곳을 함께 고쳤다.

| 파일 | 변경 |
|---|---|
| `common/util/LikeEscape.java` (신규) | `contains()` — `!`→`!!`, `%`→`!%`, `_`→`!_` 순서로 치환 후 `%…%`로 감싼다 |
| `PatientRepository` / `KcdDiseaseRepository` | 파생 쿼리(`findByNameContaining`) → `@Query`의 `like :pattern escape '!'` |
| `PatientService` (3곳), `KcdDiseaseController`, `DrugMasterController` | 호출부에서 `LikeEscape.contains(...)` 적용 |
| `DiagnosisApplicationTests` | 회귀 테스트 `patientSearchTreatsWildcardsAsLiteralCharacters()` 추가 |

> 📌 **이건 SQL 인젝션이 아니다.** JPA가 값을 파라미터로 바인딩하므로 쿼리 구조는 안전하다.
> 문제는 **`%`가 SQL LIKE에서 "아무 문자열"이라는 뜻**이라는 것이다. 검색창에 `%` 한 글자만 치면
> `name like '%%%'` 가 되어 **환자 명단 전체**가 나온다. 권한이 있는 사용자라도 "이름으로 한 명 찾기"에
> 필요한 범위를 훨씬 넘어선다 — 과다 노출이지 인젝션이 아니다.

> 📌 **이스케이프 문자로 `\`가 아니라 `!`를 골랐다.** MySQL은 백슬래시를 문자열 리터럴 수준에서
> 한 번 더 해석해서, 몇 번 겹쳐 써야 하는지가 환경(H2 테스트 vs MySQL 운영)마다 달라진다.
> `!`는 어느 쪽에서도 특별한 의미가 없어 동작이 같다.

> 📌 **치환 순서가 중요하다.** 이스케이프 문자 자신(`!`)을 **가장 먼저** 두 배로 만들어야 한다.
> `%`를 먼저 처리하면 그때 새로 생긴 `!`가 다음 단계에서 또 이스케이프돼 값이 망가진다.

> 📌 **파생 쿼리로는 불가능했다.** Spring Data의 `findByNameContaining`은 `escape` 절을 붙일 방법이 없어
> `@Query`로 바꿔야 했다. `DrugMasterRepository.findByNameKrContaining`만 남겨 뒀는데,
> 인자가 코드에 박힌 고정 문자열(Gemini 프롬프트 예시용)이라 사용자 입력이 닿지 않기 때문이다.

**검증**: `./gradlew test` — **17건 전부 통과** (`AnalysisResilienceTest` 2, `DiagnosisApplicationTests` 9, `EndpointAuthorizationTest` 6).
회귀 테스트는 `%`·`!` 단독 입력이 0건, `와일드카드테스트_` 리터럴이 정확히 2건을 반환함을 확인한다.

---

#### 배포 하드닝 (`fix/#66`, G2 외 추가 작업)

EC2 배포가 임박해 함께 처리했다. G2 항목은 아니지만 **배포 순간 곧바로 문제가 되는 것들**이다.

| 파일 | 변경 | 이유 |
|---|---|---|
| `docker-compose.prod.yml` (신규) | MySQL·backend `ports` 제거, frontend `80:80`, `SWAGGER_ENABLED=false`, `LOG_LEVEL=INFO` | 로컬에서 편하려고 열어 둔 포트가 EC2에서는 **인터넷 전체에 열린 문**이 된다 |
| `docker-compose.yml` | `DB_PASSWORD`의 `:-rootpass` 제거 → `:?`로 필수화 | JWT 서명키와 **똑같은 함정이 한 층 아래에** 남아 있었다. `.env`를 비워두면 공개 저장소에 적힌 값으로 조용히 넘어갔다 |
| `.env.production.example` (신규) | EC2 전용 템플릿 | 로컬 `.env`를 복사해 오지 말라는 경고 포함 — 로컬 값은 이미 노트북 여러 대와 채팅방을 거쳤다 |
| `.gitignore` | `!.env.production.example` 예외 추가 | `.env.*` 규칙에 걸려 커밋되지 않던 문제 |
| `docs/ec2-deployment-guide.md` (신규) | AWS 무경험자 기준 단계별 가이드 | 8단계에 HTTPS(Caddy + Let's Encrypt) 절차 포함 |

> 📌 **`ports: []`로는 포트가 지워지지 않는다.** compose는 여러 `-f` 파일을 합칠 때 목록형 값을
> **덮어쓰지 않고 이어 붙인다.** 그래서 빈 목록을 써도 기본 파일의 `3306:3306`이 그대로 살아남는다.
> `!reset`(삭제)과 `!override`(교체)를 써야 하고, **Docker Compose v2.24 이상**이 필요하다.
> 실제로 첫 시도에서 `!reset` 뒤에 `80:80`을 적었다가 프론트 포트가 통째로 사라지는 것을
> `docker compose ... config`로 확인하고 `!override`로 바로잡았다. **눈으로 확인하지 않았으면 몰랐을 종류다.**

> 📌 **남은 최대 리스크는 HTTPS다.** 지금 붙이지 않으면 이슈 #1에서 막은 토큰 위조를
> "위조할 필요 없이 평문으로 주워 가는" 경로가 열린다. 배포 가이드는 그래서
> **HTTPS 적용 전까지 보안 그룹을 팀 IP로 제한**하고, 적용 후에 전체 공개하도록 순서를 못 박았다.

#### 프론트엔드 정비 (`fix/#66`) — 백엔드 #1~#6 변경에 맞춤

이슈 #1~#6은 **서버가 거절하는 방식**을 바꿨다. 그런데 프론트는 여전히 예전 응답을 전제로 짜여 있어서,
서버가 정확히 거절해도 **화면에는 그 사실이 나타나지 않는 상태**였다.

**① 응답 계약 정정 (백엔드 선행 작업)**

| 파일 | 변경 | 이유 |
|---|---|---|
| `SecurityErrorResponder.java` (신규) | 미인증 → `401` JSON / 필터 단계 권한 거부 → `403` JSON | §2-3 하단 "401 전역 처리" 정정 참조. 이게 없으면 아래 ②~③이 전부 추측이 된다 |
| `SecurityConfig.java` | `exceptionHandling` 연결 | 필터에서 걸린 요청은 `@RestControllerAdvice`까지 오지 못한다 |
| `GlobalExceptionHandler.java` | 500 응답에서 예외 문자열 제거, 스택트레이스는 **로그로** | `NullPointerException: null` 같은 내부 정보가 사용자 화면까지 나가고 있었다. SQL·테이블명·파일 경로가 섞여 나올 수 있는 자리다 |

**② 오류 처리 공통화**

| 파일 | 변경 | 이유 |
|---|---|---|
| `api/session.ts` (신규) | 토큰 보관 + 세션 만료 신호 | `client.ts`와 `AuthedImage.tsx`가 각자 `localStorage`를 직접 읽고 있었다. `client.ts`↔`AuthContext` 순환 참조도 여기서 끊는다 |
| `api/errors.ts` (신규) | 상태코드별 한국어 문구 + `isSessionExpired`/`isForbidden`/`isNotFound` | 화면마다 복사돼 있던 `getErrorMessage`를 한 곳으로. **5xx는 서버 메시지를 쓰지 않고 덮는다**(①과 이중 방어) |
| `api/client.ts` | 빈 본문·204·네트워크 실패·깨진 JSON 처리, 401 시 만료 통보 | 빈 본문 403이 `message: ""`가 되어 **화면에 아무것도 안 뜨던** 문제. `fetch`의 `TypeError`는 `ApiError`가 아니라 호출부 검사를 빠져나갔다 |

**③ 세션 만료 대응 (전에는 아예 없었다)**

토큰이 만료되면(개발 24시간 / 운영 8시간) `localStorage`에는 로그인 정보가 남아 있어서
**화면은 로그인된 척하고 누르는 것마다 조용히 실패**했다. 매일 한 번씩 일어나는 일이다.

- `client.ts`가 `401`을 받으면 `notifySessionExpired()` → `AuthProvider`가 세션을 지움 → `PrivateRoute`가 로그인 화면으로 보냄
- 여러 요청이 동시에 401로 떨어져도 **알림은 한 번만** (화면 하나가 진료기록·이미지·분석결과를 병렬로 부른다)
- **토큰을 실제로 갖고 있던 요청만** 만료로 친다 — 로그인 전 요청까지 잡으면 로그인 화면에 엉뚱한 안내가 뜬다
- `AuthedImage`도 `apiRequest`를 거치지 않으므로 같은 처리를 따로 넣었다

**④ 화면별 정비**

| 파일 | 변경 |
|---|---|
| `Login.tsx` | `alert()` → 폼 인라인 표시. **서버가 준 사유를 그대로** ("이미 사용 중인 아이디입니다"). 중복 제출 차단, 만료 안내 배너 |
| `Clinic.tsx` / `Reception.tsx` | 각자 복사돼 있던 `getErrorMessage` 삭제 → `api/errors` 사용 |
| `Lookup.tsx` / `ClinicPatientLookupPanel.tsx` | "환자 조회 중 오류" 같은 뭉뚱그린 문구 → 실제 사유 |
| `SearchModal.tsx` | 실패를 **"검색 결과 없음"으로 보여주던 것**을 오류로 분리 |
| `KioskAnalyze.tsx` | `404`만 "유효하지 않은 QR"로, 나머지는 실제 사유 |
| `AuthedImage.tsx` | `src` 변경 시 **이전 환자 사진이 한 프레임 남던 것** 제거 (아래 📌) |
| `auth/AuthContext.ts` (신규) | 훅·타입을 컴포넌트 파일에서 분리 — Fast Refresh 복구 |
| `contexts/auth.ts`, `hooks/useAuth.ts` | 삭제 (죽은 코드) |

> 📌 **리팩토링 중에 발견한 실제 버그.** `AuthedImage`는 `useEffect` 안에서 이전 이미지를 지웠다.
> 그런데 effect는 **화면이 그려진 뒤에** 실행된다 — 즉 다른 환자를 선택한 직후 한 프레임 동안
> **이전 환자의 병변 사진이 새 환자 화면에 남아 있었다.** 상태에 `src`를 함께 저장해
> "지금 보고 있는 환자의 결과일 때만 그린다"로 바꿔 해결했다.
> 린트 규칙(`react-hooks/set-state-in-effect`)이 가리키던 자리였고, **성능 경고인 줄 알았던 것이 실제로는 의료 정보 오표시였다.**

> 📌 **`.env` 검증도 한 단위다.** 이번에 백엔드를 다시 띄우려다 컴포즈가 기동을 거부했다.
> `.env` 18번 줄에 `INTERNAL_API_SECRET=...`이 아니라 **그 값을 만드는 명령문 자체가 문자열로 저장**돼 있었다.
> 앞선 세션에서 안내한 `echo "..." >> .env` 를 실행하지 않고 붙여넣은 결과다.
> 시크릿은 **"넣었다"가 아니라 "그 값으로 서버가 떴다"까지 확인해야 한 단위**다.
...
| **(추가)** 키오스크 대상 선정 | `KioskService.findPending()`의 전체 스캔 루프를 조회 1번짜리 JPQL로 교체. **N+1 해소 + 의도 명시** — §2 (K) 참조 |
...
| 키오스크 대상 선정 | 묵은 접수가 있어도 최신 접수를 선택, 예비분석 완료 건은 건너뜀 |
| `./gradlew test` | **10/10 통과** (`AnalysisResilienceTest` 2건 + 키오스크 대상 선정 1건 신규) |
...
### 이슈 #6 — 역할 기반 인가 전면 적용 (감사 4번) · ✅ 구현 완료

```
제목: [보안] 직책(의사/간호사/접수)에 따라 할 수 있는 일을 나누기

라벨: security, backend, frontend
```

## 📝 Description

지금은 **로그인만 하면 누구든 무엇이든 할 수 있습니다.**

접수 담당으로 가입한 계정도 처방을 저장하고, 진단을 확정하고, 진료를 완료할 수 있습니다.
회원가입 화면에 직책을 고르는 칸이 있고 DB에도 `role` 값이 저장되지만,
**그 값을 확인하는 코드가 서버 어디에도 없습니다.**

`SecurityConfig`에 `@EnableMethodSecurity`는 이미 붙어 있습니다. 즉 **문을 달 준비는 끝났는데
문을 안 단 상태**입니다. `@PreAuthorize`가 코드 전체에 0개입니다.

병원에서 이건 단순한 권한 문제가 아닙니다. 처방은 의사의 법적 행위이고,
누가 처방했는지는 진료기록부에 남는 책임의 주체입니다.
이슈 #4에서 "처방자를 토큰에서 결정"하도록 고쳤지만, **접수 직원 계정으로 로그인해도
그 계정 이름으로 처방이 저장되는 상태**라 아직 절반만 막힌 셈입니다.

### 어떻게 나눌 것인가 — 누적 사다리

직책마다 별도 목록을 관리하면 새 기능이 생길 때마다 4곳을 고쳐야 하고 반드시 어긋납니다.
그래서 **위 직책이 아래 직책의 권한을 모두 포함**하는 사다리 구조로 갑니다.

| 직책 | 할 수 있는 일 |
|---|---|
| **STAFF** (접수) | 환자 등록·검색, 접수 생성, QR 발급, 조회 |
| **NURSE** (간호) | STAFF가 하는 것 전부 + 이미지 업로드, AI 분석, 진료 시작 |
| **DOCTOR** (의사) | NURSE가 하는 것 전부 + 처방 저장, 진단 확정, 진료 완료 |
| **ADMIN** (관리자) | 전부 |

> **왜 조회를 접수 직원에게도 여는가.** 접수 직원이 환자를 못 찾으면 접수를 할 수 없습니다.
> 실제 병원에서도 원무과는 차트를 조회합니다. 의료법이 요구하는 통제는 "의사만 차트를 본다"가 아니라
> **역할 기반 접근 + 접속기록**입니다 — §2 (C) 참조. 접속기록은 G3에서 이어집니다.

## ✅ To-Do List

- [x] `@StaffAccess` / `@MedicalAccess` / `@DoctorAccess` 메타 애노테이션 3개 생성
- [x] 11개 컨트롤러 전부에 적용
- [x] 무인증으로 열어둔 경로에 `@PublicEndpoint` 표식 (로그인/가입, 키오스크)
- [x] 인가 애노테이션이 없는 엔드포인트가 있으면 **빌드를 실패시키는 테스트**
- [x] 프론트: 직책이 열 수 없는 화면은 탭에서 감추고, 주소창으로 들어와도 되돌리기
- [x] 프론트: 진료 화면 안에서 처방 입력·진료 완료는 의사에게만 노출

## 참고

- `@EnableMethodSecurity`는 `SecurityConfig.java:30`에 이미 있습니다 — 설정 추가 불필요
- 권한 이름은 `JwtFilter.java:60`이 `ROLE_ + role`로 만들어 주므로 `hasAnyRole('DOCTOR')`가 그대로 맞습니다
- **소유권 검사(내가 담당한 환자만 조회)는 넣지 않습니다.** 당직·협진·인수인계가 전부 막힙니다 — §2 (C) 참조

---

#### 실제 구현 (`fix/#65`)

To-Do 6건 모두 처리했다. 그리고 **구현 중에 계획에 없던 결함 1건을 발견해 함께 고쳤다.**

| 항목 | 처리 |
|---|---|
| 메타 애노테이션 | `common/security/`에 `@StaffAccess`(STAFF~ADMIN) / `@MedicalAccess`(NURSE~) / `@DoctorAccess`(DOCTOR~) 3개. 각 파일에 "왜 이 선인가"를 적었다 |
| 컨트롤러 적용 | 11개 전부. 직책이 균일한 컨트롤러는 클래스 단위, 섞인 컨트롤러(`Visit`·`Prescription`·`Analysis`·`VisitImage`)는 메서드 단위 |
| 공개 경로 표식 | `@PublicEndpoint` — 권한을 주지 않는 **표식 전용** 애노테이션. `AuthController`·`KioskController` 2개뿐 |
| 가드 테스트 | `EndpointAuthorizationTest` 6건. ArchUnit 대신 실제 등록된 핸들러를 읽는다 — §2 (D) 참조 |
| 프론트 라우팅 | `auth/roles.ts`(사다리 + 화면별 최소 직책 1곳) + `RoleRoute` + `MainLayout` 탭 필터링 |
| 프론트 진료 화면 | 간호사는 이미지 업로드·AI 분석까지 하고, 처방 폼과 진료 완료 버튼은 보이지 않는다 |
| **(발견·수정)** 인가 거부가 500 | `AccessDeniedException` → **403** 핸들러 신설. 아래 참조 |

> 📌 **인가를 걸었더니 403이 아니라 500이 나왔다.**
> 메서드 시큐리티가 던지는 `AuthorizationDeniedException`은 **컨트롤러 호출 시점**에 발생하므로
> `GlobalExceptionHandler`(@RestControllerAdvice)가 먼저 낚아채고, 해당 핸들러가 없어
> 맨 아래 `handleEtc(Exception)` → 500으로 떨어지고 있었다.
> Spring Security가 403으로 바꿔주는 `ExceptionTranslationFilter`는 **필터 레벨**이라 여기까지 오지 못한다.
> 500으로 나가면 (a) 프론트가 "권한 없음"과 "서버 고장"을 구분하지 못하고,
> (b) 모니터링에서 **정상적인 권한 차단이 장애 알람**으로 잡힌다.
> → `handleAccessDenied`를 추가해 403 + 한국어 메시지로 정정하고, **누가 무엇을 시도했는지 WARN 로그**를 남긴다.
> 이 로그는 G3 감사 로그의 씨앗이기도 하다.

> 📌 **`@PublicEndpoint`를 왜 굳이 만들었나.** 가드 테스트에 "이 경로들은 예외" 목록을 문자열로 박아두면
> 코드와 목록이 조용히 어긋난다. 대신 **공개하겠다는 결정을 코드에 남기게** 했다.
> 다만 이것만으로는 "귀찮으니 일단 붙이자"를 막지 못하므로, 테스트가 **공개 컨트롤러 목록까지 함께 고정**한다 —
> 공개 대상을 늘리려면 테스트 파일을 고쳐야 하고, 그러면 리뷰에서 반드시 눈에 띈다.

> 📌 **`@Valid`가 `@PreAuthorize`보다 먼저 돈다.** 검증에 걸리는 body를 보내면 403이 아니라 400이 나온다
> (Spring MVC 순서: 인자 바인딩·검증 → 컨트롤러 호출을 감싼 시큐리티 인터셉터).
> 보안상 문제는 아니다 — 권한 없는 요청이 통과하는 게 아니라 순서만 그렇다.
> 다만 **인가 테스트는 반드시 검증을 통과하는 body로 짜야** 한다. 처음에 빈 배열을 보냈다가
> 400이 나와 "인가가 안 걸렸나" 하고 한 번 헤맸다.

**검증 결과** (로컬 도커 실서버 + `./gradlew test`)

STAFF/NURSE/DOCTOR 계정을 실제로 만들어 10개 엔드포인트 × 3직책 = 30건을 호출했다.
`403`은 차단, 그 외(`200`/`404`)는 인가를 통과했다는 뜻이다(대상 접수가 없어 404).

| 엔드포인트 | STAFF | NURSE | DOCTOR |
|---|---|---|---|
| 환자 검색 / 접수 목록 / 상병코드 검색 | 200 | 200 | 200 |
| AI 분석 요청 · 진료 시작 · 예비분석 조회 | **403** | 404 | 404 |
| 처방 저장 · 진단 확정 · 진료 완료 | **403** | **403** | 404 |
| 내 처방 환자 목록 | **403** | **403** | 200 |

| 확인 항목 | 결과 |
|---|---|
| 30건 전수 | **전부 기대대로** |
| 무인증 호출 | 403 (차단) |
| 키오스크 공개 경로 | 404 — 인증은 통과, 토큰이 없을 뿐 (공개 경로가 막히지 않았음을 확인) |
| `./gradlew test` | **16/16 통과** (`EndpointAuthorizationTest` 6건 신규) |
| `npm run build` | 통과 |
| 검증용 계정 | 3건 생성 → **전량 삭제 확인** |

> 검증에 쓴 계정 3개는 실행 후 DB에서 지웠다(`member` 잔여 3건 = 기존 계정만).

**남은 것**

- `GET /api/v1/prescriptions/doctor-patients`는 **프론트에서 아직 아무도 부르지 않는다.** 백엔드만 있는 상태다.
- 무인증 호출이 **401이 아니라 403**으로 나간다. Spring Security 기본 진입점(`Http403ForbiddenEntryPoint`) 동작이며
  이번 변경과 무관한 기존 동작이다. "인증이 없음(401)"과 "권한이 부족함(403)"을 구분하려면
  `AuthenticationEntryPoint`를 따로 지정해야 한다 — 프론트 재로그인 유도 로직과 함께 봐야 하므로 별도 이슈로 남긴다.
...
> **G2를 절반쯤 진행하고 확인된 것**: 위 경고("이슈에 적힌 것만 고치고 닫지 말 것")가 두 번 더 맞았다.
> #5에서는 계획에 없던 `KioskService`의 `break` 누락을, #6에서는 **인가 거부가 403이 아니라 500으로 나가던 결함**을
> 함께 찾았다. 두 건 모두 "고치려던 것"을 실제로 돌려 보는 과정에서 드러났다 —
> 코드를 읽어서가 아니라 **실서버에 요청을 던져서** 나왔다는 공통점이 있다.
> 남은 G2·G3 항목도 구현 후 반드시 실호출로 확인할 것.
>
> 한편 이슈 #6으로 **인가의 뼈대는 완성됐다.** G0(#1)이 "누구인가"를 위조 불가능하게 만들었고,
> #3이 "인증 없이는 아무것도"를, #4가 "처방자는 토큰이 결정"을, #6이 "직책별로 할 수 있는 일"을 세웠다.
> 이제 남은 축은 **"누가 무엇을 했는가"(G3 감사 로그)** 하나다.
> #6이 추가한 권한 거부 WARN 로그가 그 첫 줄이 된다.

> **G2를 닫으며**: 잔여 2건(FastAPI 시크릿, LIKE 이스케이프)까지 처리해 **G2가 전부 완료**됐다.
> 여기서도 같은 패턴이 반복됐다 — 계획서는 LIKE 결함을 환자 검색 한 곳으로 적었지만
> 실제로는 **질병·약품 검색에도 같은 코드가 있었다.** 계획서의 항목은 "여기부터 보라"는 출발점이지
> 목록의 끝이 아니라는 것이 이제 네 번째로 확인됐다.
>
> 그리고 이번 작업의 절반은 코드가 아니라 **배포 설정**이었다. `ports: []`로는 포트가 닫히지 않는다는 것을
> `docker compose config`로 직접 확인하기 전까지 몰랐다는 점이 시사적이다. 보안 설정은
> **"썼다"가 아니라 "적용됐는지 확인했다"까지가 한 단위**다. 배포 가이드의 7-4단계에
> `docker ps`로 열린 포트를 눈으로 세는 절차를 넣은 이유가 이것이다.
>
> **남은 최우선 과제는 HTTPS(G4)다.** 배포 자체는 지금 상태로 가능하지만,
> HTTP로 열어 두면 #1에서 막은 토큰 위조가 "평문으로 주워 가기"로 대체될 뿐이다.
> 그래서 배포 가이드는 **HTTPS 전에는 보안 그룹을 팀 IP로 제한**하는 2단계 공개 절차를 강제한다.
> 그다음이 CI, 그다음이 G3 감사 로그다.
