 ...
 * 제증명 발급·조회·재발급·무효 처리.
 *
 * <p>이 서비스가 지키는 규칙은 셋이다.
 *
 * <ol>
 *   <li><b>의료법 제17조</b> — 진단명이 들어가는 서류는 직접 진찰한 의사만 낼 수 있다.
 *       로그인한 사람이 그 내원의 처방을 작성한 본인인지 확인한다. 같은 병원 의사라도 남의 환자
 *       진단서를 대신 떼줄 수는 없다.</li>
 *   <li><b>사실은 서버가 채운다</b> — 병명·약품·날짜·면허번호는 요청에서 받지 않고 DB에서 조립한다.</li>
 *   <li><b>발급한 것은 지우지 않는다</b> — 잘못 나간 서류도 행을 삭제하지 않고 무효 표시만 한다.
 *       발급대장에서 기록이 사라지면 그 서류가 존재했다는 사실 자체를 확인할 수 없게 된다.</li>
 * </ol>
 */
@Service
...
public class CertificateService {
     ...
     *
     * <p>지금 데이터로 다시 만들지 않고 <b>원본 스냅샷을 그대로 복사</b>한다. 그 사이 처방이 수정됐다면
     ...
     * 문서에 서명될 의사를 정한다.
     *
     * <p>발급 버튼을 누른 사람과 다를 수 있다. 원무과 직원이 처방전을 재출력하거나 진료확인서를
