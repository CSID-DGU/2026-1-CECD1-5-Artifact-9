 ...
 *
 * <p>서류 5종이 필드를 공유한다. 종류마다 쓰는 칸이 다를 뿐이라 안 쓰는 칸은 null 로 둔다.
 * 종류별로 record 를 따로 두지 않은 이유는, 저장된 JSON을 다시 읽어 출력할 때
 * 종류에 따라 역직렬화 타입을 갈아끼우는 분기가 프론트·백엔드 양쪽에 생기기 때문이다.
 *
 * <p>여기 담긴 값은 <b>발급 당시의 사실</b>이다. 나중에 환자 이름이 바뀌든 처방이 수정되든
 * 이 스냅샷은 건드리지 않는다. 재발급은 이 값을 그대로 다시 렌더링하는 것이다.
 *
 * <p>{@code patientResidentNo} 는 마스킹된 표시용 문자열이다(예: {@code 900101-1******}).
