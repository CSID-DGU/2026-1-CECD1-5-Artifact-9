...
/**
 * 분석 1건이 <b>실제로 모델에 넣은</b> 이미지 매핑.
 * DB 테이블: analysis_image
 *
 * <p><b>왜 이 테이블이 필요한가.</b> {@link AnalysisResult} 에는 이미지 컬럼이 하나도 없다 —
 * {@code visit_id} 뿐이다. 그런데 {@code visit_image} 는 접수 1건에 사진이 여러 장 붙는 구조라,
 * 이 매핑이 없으면 <b>"이 진단이 어느 사진에서 나왔는가"에 DB가 답할 수 없다.</b>
 * 모델 버전은 가중치 해시로 정확히 남는데 정작 입력이 무엇이었는지가 비면 감사 추적이 반쪽이 된다.
 *
 * <p><b>왜 복합 PK (analysis_id, image_id) 인가.</b> 처음부터 N:M 을 상정했기 때문이다.
 * <ul>
 *   <li>한 분석이 여러 장을 함께 본다 — 같은 병변의 여러 각도. 다중 입력 모델로 확장할 때 쓴다.</li>
 *   <li>한 장이 여러 분석에 쓰인다 — 모델을 교체한 뒤 같은 사진을 재분석하는 경우.
 *       {@code AnalysisResult} 의 "같은 이미지를 여러 번 재분석할 수 있음"이 이것이다.</li>
 * </ul>
 * 그래서 이 클래스는 지금 당장 1행만 쌓이더라도 <b>리스트를 받는 경로</b>로 저장된다.
 * 확장 시 {@link AnalysisService} 의 {@code selectInferenceTargets} 한 곳만 고치면 된다.
 *
 * <p><b>2026-08-30 기록.</b> 이 테이블은 2026-05-17(e31e890) 스키마 v0.4 에서
...
public class AnalysisImage {
     ...
     * 복합 PK 식별자.
     *
     * <p>JPA 규약상 {@code @IdClass} 는 {@code Serializable} 이어야 하고 equals/hashCode 가
