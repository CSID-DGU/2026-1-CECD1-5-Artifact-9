 ...
 *
 * <p><b>왜 필요한가.</b> SQL의 {@code LIKE}에서 {@code %}는 "아무 글자나 몇 개든",
 * {@code _}는 "아무 글자 하나"를 뜻한다. 검색어를 그대로 {@code "%" + 입력 + "%"} 로 감싸면
 * 사용자가 친 {@code %} 가 와일드카드로 살아난다 — 환자 검색창에 {@code %} 한 글자만 넣으면
 * <b>전체 환자 명단이 그대로 나온다.</b> 이름을 몰라도 목록을 통째로 받아낼 수 있다는 뜻이다.
 *
 * <p>SQL 인젝션은 아니다. JPA가 값을 파라미터로 바인딩하므로 쿼리 구조는 바뀌지 않는다.
 * 하지만 <b>의도한 것보다 훨씬 많은 데이터가 나간다</b>는 점에서 결과는 비슷하게 나쁘다.
 *
 * <p><b>이스케이프 문자로 {@code !} 를 쓰는 이유.</b> 흔히 쓰는 백슬래시는 MySQL이
 ...
 *
 * <p>이 클래스를 쓰는 쿼리는 반드시 {@code escape '!'} 절을 함께 적어야 한다.
...
public final class LikeEscape {
     ...
     *
     * <p>빈 문자열이나 {@code null} 은 "전체 조회"가 된다. 이는 기존 동작 그대로이며
     * 이 클래스가 바꾸려는 대상이 아니다 — 여기서 막는 것은 <b>검색어 안에 숨은 와일드카드</b>다.
     */
     ...
     *
     * <p>순서가 중요하다. 이스케이프 문자 자신({@code !})을 <b>가장 먼저</b> 치환해야 한다.
