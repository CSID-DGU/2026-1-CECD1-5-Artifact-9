 ...
 * CORS 설정.
 *
 * <p><b>왜 same-origin 배포인데도 이 목록이 필요한가.</b> 브라우저는 주소창과 같은 출처로 보내는
 ...
 * <p>구성:
 * <ul>
 *   <li>Docker nginx 경유(localhost:3000 / Mac LAN IP:3000) — 직접 API 호출과 Swagger UI 용.</li>
 *   <li>로컬 Vite dev(localhost:5173) — vite proxy 가 처리하지만 목록에 포함한다.</li>
 *   <li>대기실 iPad 키오스크 시연 — 같은 Wi-Fi 의 사설 IP 로 접속하므로 RFC1918 대역을 허용한다.</li>
 *   <li>배포 도메인 — {@code cors.allowed-origins} 로 주입한다(application.properties 참고).
 *       도메인이 바뀌면 코드가 아니라 {@code CORS_ALLOWED_ORIGINS} 환경변수를 고친다.</li>
 * </ul>
 */
@Configuration
