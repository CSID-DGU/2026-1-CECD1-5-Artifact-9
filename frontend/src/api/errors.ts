 ...
 *
 * <p>전에는 화면마다 `error instanceof Error ? error.message : "요청 처리 중 오류가 발생했습니다."`를
 * 각자 복사해 두고 있었다(Clinic.tsx, Reception.tsx). 두 가지가 문제였다:
 *
 * <ul>
 *   <li>서버가 본문 없이 상태 코드만 돌려주면 `error.message`가 빈 문자열이라
 *       <b>화면에 아무것도 안 뜬다.</b> 사용자는 버튼을 눌렀는데 반응이 없는 것처럼 본다.</li>
 *   <li>500 응답의 메시지가 `NullPointerException: null` 같은 자바 내부 정보였다.
 *       사용자에게 의미도 없고 내부 구조만 노출된다.</li>
 * </ul>
 *
 * <p>그래서 규칙을 하나로 정한다 —
 * <b>서버가 사람이 읽을 메시지를 줬으면 그걸 쓰고, 아니면 상태 코드에 맞는 기본 문구를 쓴다.</b>
...
const FALLBACK_BY_STATUS: Record<number, string> = {
  ...
  422: "이미지를 분석할 수 없습니다. 병변이 잘 보이도록 다시 촬영해 주세요.",
