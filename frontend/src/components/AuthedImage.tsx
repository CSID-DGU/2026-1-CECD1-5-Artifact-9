 ...
 * 인증이 필요한 이미지를 표시한다.
 *
 * 브라우저의 &lt;img src&gt;는 Authorization 헤더를 실을 수 없다. 그래서 서버가 환자 이미지를
 * permitAll로 열어두는 방식으로 굴러가고 있었는데, 그 경로는 visitId/imageId가 순차 정수라
 * 번호만 훑으면 전체 환자의 병변 사진을 수집할 수 있었다.
 *
 * 이제 이미지 API도 인증을 요구하므로, fetch로 Bearer 토큰을 붙여 받아온 뒤
 * Blob URL을 만들어 &lt;img&gt;에 넣는다. 만든 Blob URL은 언마운트·src 변경 시 해제한다 —
 * 해제하지 않으면 이미지 바이트가 탭이 닫힐 때까지 메모리에 남는다.
 *
 * 결과에 src를 같이 저장하는 이유. 전에는 `objectUrl` 상태만 두고 effect 안에서
 * `setObjectUrl(null)`로 초기화했다. 그런데 effect는 화면이 그려진 뒤에 실행된다 —
 * 즉 다른 환자를 선택한 직후 한 프레임 동안 이전 환자의 병변 사진이 그대로 남아 있었다.
 * 지금은 저장된 결과의 src가 현재 src와 같을 때만 그리므로, 그 프레임이 생기지 않는다.
 *
 * 401은 다른 실패와 구분한다. 이 컴포넌트는 apiRequest를 거치지 않고 fetch를
 * 직접 부르므로, 여기서 알려주지 않으면 토큰이 만료돼도 앱이 알지 못한다.
